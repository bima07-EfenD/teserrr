    ...
    <div class="min-h-screen bg-gray-50" x-data="{
        ...
        <div class="fixed inset-y-0 left-0 z-50 w-64 bg-white shadow-xl transform transition-all duration-300 ease-in-out"
            ...
            <div class="flex items-center justify-between px-6 py-5 border-b border-gray-100">
                ...
                <button @click="sidebarOpen = false" class="lg:hidden text-gray-500 hover:text-gray-700 transition-colors duration-200">
            ...
            <nav class="flex-1 px-4 py-4 space-y-2 overflow-y-auto">
                <a href="{{ route('owner.dashboard') }}"
                   class="{{ request()->routeIs('owner.dashboard') ? 'bg-blue-50 text-blue-600 border-blue-100' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900 border-transparent' }}
                ...
                <a href="{{ route('owner.pegawai.index') }}"
                   class="{{ request()->routeIs('owner.pegawai.*') ? 'bg-blue-50 text-blue-600 border-blue-100' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900 border-transparent' }}
                          flex items-center px-4 py-3 text-sm font-medium rounded-xl border transition-all duration-200 group">
                    <svg class="w-5 h-5 mr-3 transition-colors duration-200" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" />
                    </svg>
                    <span class="transition-colors duration-200">Pegawai</span>
                </a>

                <a href="{{ route('owner.laporan.index') }}"
                   class="{{ request()->routeIs('owner.laporan.*') ? 'bg-blue-50 text-blue-600 border-blue-100' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900 border-transparent' }}
                          flex items-center px-4 py-3 text-sm font-medium rounded-xl border transition-all duration-200 group">
                    <svg class="w-5 h-5 mr-3 transition-colors duration-200" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <a href="{{ route('owner.akun.index') }}"
                   class="{{ request()->routeIs('owner.akun.*') ? 'bg-blue-50 text-blue-600 border-blue-100' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900 border-transparent' }}
                          flex items-center px-4 py-3 text-sm font-medium rounded-xl border transition-all duration-200 group">
                    <svg class="w-5 h-5 mr-3 transition-colors duration-200" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                    </svg>
                    <span class="transition-colors duration-200">Akun</span>
                </a>
